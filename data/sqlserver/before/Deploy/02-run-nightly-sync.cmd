...
echo Running nightly sync bridge on %LEGACY_SQL_SERVER%.
sqlcmd -b -l 5 -S "%LEGACY_SQL_SERVER%" -d master %SQLCMD_AUTH% -i "%SCRIPT_DIR%..\\..\\shared\\Migration\\01-run-nightly-sync.sql" -v StoreOpsDatabase="%StoreOpsDatabase%" CustomerHubDatabase="%CustomerHubDatabase%" ReportingDatabase="%ReportingDatabase%"
exit /b %errorlevel%
