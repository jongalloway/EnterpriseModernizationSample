...
echo Deploying Fabrikam Enterprise Pizza databases on %LEGACY_SQL_SERVER%.
sqlcmd -b -l 5 -S "%LEGACY_SQL_SERVER%" -d master %SQLCMD_AUTH% -i "%SCRIPT_DIR%00-deploy-all.sql" -v StoreOpsDatabase="%StoreOpsDatabase%" CustomerHubDatabase="%CustomerHubDatabase%" ReportingDatabase="%ReportingDatabase%"
exit /b %errorlevel%
