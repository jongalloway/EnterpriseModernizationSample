...
echo Capturing deployment audit from %LEGACY_SQL_SERVER%.
sqlcmd -b -l 5 -S "%LEGACY_SQL_SERVER%" -d master %SQLCMD_AUTH% -i "%SCRIPT_DIR%..\\..\\shared\\Migration\\03-deployment-audit.sql" -v StoreOpsDatabase="%StoreOpsDatabase%" CustomerHubDatabase="%CustomerHubDatabase%" ReportingDatabase="%ReportingDatabase%"
exit /b %errorlevel%
