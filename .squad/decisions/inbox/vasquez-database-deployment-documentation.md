...
- **Addendum (2026-05-14T22:58:44.433+02:00):** The wrappers now share one helper, treat the environment file as the source of truth for SQLCMD variables, and lean on `SQLCMDPASSWORD` instead of a `-P` switch so the deployment story stays legacy-real without splashing passwords into the process list.
