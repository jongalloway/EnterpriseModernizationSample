...
- 2026-05-18T01:39:47.894-07:00: Partner service work lands best when `Fabrikam.EnterprisePizza.Services.PartnerSync` hosts both a `basicHttpBinding` WCF `PartnerService.svc` surface and a separate `PartnerLookup.asmx` compatibility surface over the same CustomerHub service/repository seam.
- 2026-05-18T01:39:47.894-07:00: CustomerHub partner workflows read truest when `IPartnerAccountService` wraps the repository through an Enterprise Library-style boundary handler, while `LegacyDbGateway` keeps the service contract honest with stored-procedure-shaped partner, referral, and commission datasets.
- 2026-05-18T01:39:47.894-07:00: StoreOps seed data lands best as one operational seed stack plus separate `03-menu-products.sql`, `03-store-configuration.sql`, and `03-delivery-records.sql` files wired into `data\sqlserver\before\Deploy\00-deploy-all.sql`, so DBAs can review roster/order data separately from menu, config, and completed-delivery history.
- 2026-05-18T01:39:47.894-07:00: Local validation for Issue #15 passed by replaying `data\sqlserver\before\StoreOps\01-create-database.sql` through `04-service-procedures.sql` against `(localdb)\MSSQLLocalDB`, producing 8 stores, 12 drivers, 72 orders, 16 delivery records, and 8 store configuration rows.
- 2026-05-18T01:39:47.894-07:00: CustomerHub DAL work lands best when repository wrappers stay split by partners, accounts, contracts, and referrals while all of them validate the shared `FabrikamPizza_CustomerHub` connection through a dedicated Enterprise Library database factory seam.
- 2026-05-18T01:39:47.894-07:00: CustomerHub seed data reads truest when base partner/account rows stay in the original seed and deeper B2B tiers, contract-status coverage, referrals, and commission history arrive as follow-on numbered SQL scripts wired into `data\sqlserver\before\Deploy\00-deploy-all.sql`.
