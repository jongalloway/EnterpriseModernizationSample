...
- 2026-05-18T01:39:47.894-07:00: Reporting schema issue #14/#17 landed best by seeding `DimStore`, `DimDriver`, and `DimPartner` from StoreOps and CustomerHub, then hanging delivery, order, and partner-settlement analytics off explicit star-schema facts in `data\sqlserver\before\Reporting\02-schema.sql` and `03-seed-data.sql`.
- 2026-05-18T01:39:47.894-07:00: Keep the legacy summary tables beside the new Reporting star schema so deployment audit checks and service procedures stay alive while PR #93 and SSRS consumers pivot to the `Dim*`/`Fact*` tables.
