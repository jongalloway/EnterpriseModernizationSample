...
- 2026-05-18T01:39:47.894-07:00: SSRS lands best as a downstream seam inside `src\before\Fabrikam.EnterprisePizza.Reporting.Batch`, where `ReportExecution2005.asmx` SOAP envelopes, shared `FabrikamPizza_Reporting` data-source config, and checked-in `.rdl` assets travel together for batch-friendly deployment.
- 2026-05-18T01:39:47.894-07:00: Partner profitability reporting needed the missing `dbo.PartnerProfitabilitySummary` table added to `data\sqlserver\before\Reporting\02-schema.sql` so report definitions, seed data, and nightly rebuild procedures all point at the same Reporting-owned settlement snapshot.
