...
- 2026-05-18T01:39:47.894-07:00: The dispatch SOAP seam lands best as a second contract inside `Fabrikam.EnterprisePizza.Services.DispatchHost`, reusing `Shared.Contracts\Routing` DTOs and EntLib host wiring instead of spawning a one-off service project for route planning and driver assignment.
- 2026-05-18T01:39:47.894-07:00: `UpdateDeliveryStatus` reads truest when the WCF endpoint can seed a `DeliveryTrackingRecord` from a `DispatchTicket` on first contact, then hand later transitions back through `DispatchCoordinator`, because the client keeps SOAP-friendly DTOs without forcing a separate tracking-bootstrap call.
