...
- 2026-05-18T01:39:47.894-07:00: Reporting DAL issue #20 landed best as a warehouse seam in `Fabrikam.EnterprisePizza.Data` with separate dimension repositories, fact repositories, and a star-schema join repository, all returning typed `DataSet` wrappers rather than leaking ad-hoc `DataTable` shapes.
- 2026-05-18T01:39:47.894-07:00: The connection story reads truest when `LegacyDbGateway` warms Enterprise Library `DatabaseProviderFactory` entries from config-backed `FabrikamPizza_Reporting` names, while batch and test hosts carry LocalDB-style connection strings in their `.config` files for smoke validation.
