...
- 2026-05-15T08:40:39.286+02:00 — Once a stacked dependency PR merges, restack the child branch by rebasing onto `origin/main`, force-pushing with lease, retargeting the PR to `main`, and rerunning `dotnet msbuild src\before\Fabrikam.EnterprisePizza.Legacy.sln /t:Restore /p:RestorePackagesConfig=true /p:Configuration=Debug`, `dotnet msbuild src\before\Fabrikam.EnterprisePizza.Tests.Unit\Fabrikam.EnterprisePizza.Tests.Unit.csproj /t:Build /p:Configuration=Debug`, and `powershell -ExecutionPolicy Bypass -File .\scripts\Invoke-LegacyNUnit.ps1 -AssemblyPath .\src\before\Fabrikam.EnterprisePizza.Tests.Unit\bin\Debug\Fabrikam.EnterprisePizza.Tests.Unit.dll`; PR #71 stayed reviewable with a single post-rebase commit and 33 passing local tests.
...
- 2026-05-15T06:02:37.407+02:00 — PR #69 follow-up tightened `StoreOperationsWorkbenchServiceFixture` by asserting normalized store numbers at the `DispatchCoordinator` seam instead of reusing an already-normalized `"031"` input.
- 2026-05-15T06:02:37.407+02:00 — In `WorkforceReportingServiceFixture`, capturing the stubbed `weeksBack` argument is a better reviewer-proof pattern than adding dead stub properties; it proves `WorkforceReportingService` still requests a four-week overtime trend window.
- 2026-05-15T08:40:39.286+02:00 — DAL coverage for issue #42 fits best as repository-level NUnit fixtures in `src\before\Fabrikam.EnterprisePizza.Tests.Unit\Repositories\...`, using the real `LegacyDbGateway` stubs to validate stored-procedure constants and `DataSet` row mapping without introducing mocks.
- 2026-05-15T08:40:39.286+02:00 — `Fabrikam.EnterprisePizza.Tests.Unit` must add an explicit `Fabrikam.EnterprisePizza.Core` project reference before repository fixtures can compile against `WorkforceReportRepository` return types like `LaborCostSummary`, `OvertimeTrendPoint`, `TurnoverSummary`, and `StaffingSummary`.
- 2026-05-15T08:40:39.286+02:00 — Current NUnit execution reality is `dotnet msbuild src\before\Fabrikam.EnterprisePizza.Legacy.sln /t:Restore /p:RestorePackagesConfig=true`, then `dotnet msbuild src\before\Fabrikam.EnterprisePizza.Tests.Unit\Fabrikam.EnterprisePizza.Tests.Unit.csproj /t:Build /p:Configuration=Debug`, then `powershell -ExecutionPolicy Bypass -File .\scripts\Invoke-LegacyNUnit.ps1 -AssemblyPath .\src\before\Fabrikam.EnterprisePizza.Tests.Unit\bin\Debug\Fabrikam.EnterprisePizza.Tests.Unit.dll`.
- 2026-05-18T01:39:47.894-07:00 — DAL tests stay reviewer-friendly when `LegacyDbGateway` remains overridable for recording stubs, `LegacyDatabaseFactory` resolves Enterprise Library databases by catalog name, and `Fabrikam.EnterprisePizza.Tests.Unit` carries the connection-string test config needed to exercise factory resolution without a live SQL Server.
